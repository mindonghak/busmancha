...
같은 수집 주기에서 기상청 초단기실황 API도 호출해 동탄/화성, 강남, 서울역, 잠실 날씨를 `weather_history`에 저장합니다.
새로 저장되는 좌석 데이터에는 `weather_area_key`와 `weather_area_name`을 함께 저장해 날씨별 통계 조회가 가능하도록 합니다.
